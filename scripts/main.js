...
window.onload = function () {
    ...
    */
    ...
    // Se l'elemento esiste, aggiungiamo la classe 'active' per evidenziarlo sia nel menu desktop che in quello mobile.
    if (aboutLink) {
...
// Aggiungiamo un event listener al pulsante e ogni volta che l'utente clicca sul bottone di apertura del menu mobile, eseguiamo la funzione.
toggleButton.addEventListener('click', () => {
    /* 
    Verifichiamo lo stato attuale del pulsante di apertura del menu mobile.
    Si controlla se il menu non ha ancora la classe open e se non ce l’ha, isOpening diventa true, quindi significa che il prossimo click lo sta per aprire. 
    */
    ...
    // Si gestisce il pulsante in base allo stato se è aperto o chiuso per cambiare il valore del pulsante stesso.
    toggleButton.textContent = isOpening ? '✖' : '☰';
    toggleButton.classList.add('changing');   // Aggiungiamo una classe che anima il pulsante.
    toggleButton.setAttribute('aria-expanded', isOpening);

    // Dopo un piccolo ritardo cambiamo la X nell'icona del menu con la relativa animazione.
    setTimeout(() => { 
        toggleButton.textContent = isOpening ? '✕' : '☰';
